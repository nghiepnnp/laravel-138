...
<form style="width:90%;margin:0 auto" method="post" action="{{route('singlepage_postupdate',['id'=>$row->id])}}" 
...
<fieldset>
	...
	<div class="form-group">
		...
		<input type="text" class="form-control w-100" id="formGroupExampleInput" placeholder="Name page ...  " name="name"   value="{{old('name', $row->name)}}" >
	</div>

	<div class="mb-2">
		<label  for="inlineFormCustomSelect">Chi tiết</label>
		<textarea name="content" id="editor">{{old('content',$row->content)}}</textarea>
	</div>
</div>
<br>
