...
<table id="example" class="display table" width="100%" data-page-length="10">
	<thead>
		<tr>
			...
			<th style="width:" class="text-center">Họ tên</th>
	...
	<tbody>
		...
		<tr>
			...
			<td class="pl-5" style="width:20px"><img src="{{asset('images/user/'.$l->image)}}" alt="" width="50"></td> 
			<td class="text-center">{{$l->name}}</td>
			<td class="text-center"> {{$l->email}} </td>
			<td>{{$l->phone}}</td>
			<td>{{$l->address}}</td>
			<td class="text-center">
				<a href="{{route('user_retrash',['id' => $l->id])}}" class="btn btn-primary btn-sm">
					<i class="fa fa-pen py-1"></i>
				</a>
			</td>
			<td class="text-center">
				<a href="{{route('user_delete',['id' => $l->id])}}" class="btn btn-danger btn-sm">
